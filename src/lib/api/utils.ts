...
export class ApiResponse {
  ...
  static paginated<T>(data: T[], total: number, page: number, limit: number, status = 200) {
...
export function handleApiError(error: unknown): NextResponse {
  ...
  if (error instanceof ZodError) {
    return ApiResponse.error("Invalid request data", 400, "VALIDATION_ERROR");
  }
  ...
  return ApiResponse.error("An unexpected error occurred", 500, "UNKNOWN_ERROR");
}
