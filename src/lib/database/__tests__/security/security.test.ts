...
describe('Security Tests', () => {
  ...
  describe('Cross-Tenant Access Prevention', () => {
    it('should prevent direct ID access across tenants', async () => {
      ...
      const unauthorizedUser = await userQueries.getUserById(
        authContextA,
        userClinicB.id
      )
    ...
    it('should prevent parameter manipulation attacks', async () => {
      ...
      await expect(
        clinicQueries.getClinicById(authContext, maliciousClinicId)
      ).resolves.toBeNull()
    ...
    it('should validate UUID parameters', async () => {
      ...
      for (const invalidId of invalidIds) {
        if (invalidId !== null && invalidId !== undefined) {
          await expect(
            userQueries.getUserById(authContext, invalidId)
          ).resolves.toBeNull()
        }
      }
    })
  ...
  describe('Token Security', () => {
    ...
    it('should require admin role to access tokens', async () => {
      ...
      const withToken = await googleSheetsQueries.getDataSourceById(
        adminContext,
        dataSource.id,
        { includeToken: true }
      )
