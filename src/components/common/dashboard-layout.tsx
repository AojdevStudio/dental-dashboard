...
export function MainContent({ children, className, hasSidebar = true }: MainContentProps) {
  return (
    <main
      id="main-content"
      class={cn('flex-1 overflow-y-auto bg-background', hasSidebar && 'lg:pl-0', className)}
    >
      <div class="container mx-auto px-4 py-6 lg:px-8">{children}</div>
    </main>
  );
}
...
export function DashboardLayout({ children, className }: DashboardLayoutProps) {
  ...
  return (
    <div class={cn('flex h-screen overflow-hidden', className)}>
      {/* Sidebar */}
      <Sidebar />
