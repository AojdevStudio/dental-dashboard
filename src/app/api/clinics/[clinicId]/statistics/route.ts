...
export type GetClinicStatisticsResponse = Awaited<ReturnType<typeof clinicQueries.getClinicStatistics>>
...
export const GET = withAuth(async (request, { authContext, params }) => {
  ...
  try {
    const statistics = await clinicQueries.getClinicStatistics(
      authContext,
      clinicId,
      { startDate, endDate }
    )
