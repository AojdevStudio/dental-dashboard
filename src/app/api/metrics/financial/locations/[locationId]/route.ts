...
export async function PUT(
  ...
  try {
    ...
    if (recordId) {
      ...
      const netProduction = Number.parseFloat(production || existingRecord.production.toString()) - 
      ...
      const totalCollections = Number.parseFloat(patientIncome || existingRecord.patientIncome.toString()) + 
                              Number.parseFloat(insuranceIncome || existingRecord.insuranceIncome.toString())

      const updateData: any = { netProduction, totalCollections }
      ...
      if (insuranceIncome !== undefined) updateData.insuranceIncome = Number.parseFloat(insuranceIncome)
      if (unearned !== undefined) updateData.unearned = unearned ? Number.parseFloat(unearned) : null
      ...
      const netProduction = Number.parseFloat(production || '0') - 
                           Number.parseFloat(adjustments || '0') - 
                           Number.parseFloat(writeOffs || '0')
      
      const totalCollections = Number.parseFloat(patientIncome || '0') + 
                              Number.parseFloat(insuranceIncome || '0')
