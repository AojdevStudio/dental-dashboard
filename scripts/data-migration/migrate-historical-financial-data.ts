...
async function migrateHistoricalFinancialData() {
  ...
  try {
    ...
    console.log(`📋 Found ${dentistProduction.length} dentist production records with location data`)
    ...
    for (const record of dentistProduction) {
      try {
        ...
        if (record.verifiedProductionHumble && parseFloat(record.verifiedProductionHumble.toString()) > 0) {
          ...
          if (!existingHumble) {
            ...
            recordsCreated++
            console.log(`  ✅ Created Humble record for ${recordDate}: $${record.verifiedProductionHumble}`)
          } else {
            recordsSkipped++
        ...
        if (record.verifiedProductionBaytown && parseFloat(record.verifiedProductionBaytown.toString()) > 0) {
          ...
          if (!existingBaytown) {
            ...
            recordsCreated++
            console.log(`  ✅ Created Baytown record for ${recordDate}: $${record.verifiedProductionBaytown}`)
          } else {
            recordsSkipped++
    ...
    sampleRecords.forEach(record => {
      console.log(`  - ${record.location.name} ${record.date.toISOString().split('T')[0]}: $${record.production}`)
    })
