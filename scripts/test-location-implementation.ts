...
async function testLocationImplementation() {
  ...
  try {
    ...
    locations.forEach(loc => {
      console.log(`   - ${loc.name} (${loc.clinic.name}): ${loc._count.providers} providers, ${loc._count.financials} financial records`)
    })
    ...
    providerLocations.forEach(pl => {
      console.log(`   - ${pl.provider.name} (${pl.provider.providerType}) ↔ ${pl.location.name} ${pl.isPrimary ? '(Primary)' : ''}`)
    })
    ...
    const testData = {
      ...
      adjustments: 200.00,
      writeOffs: 100.00,
      patientIncome: 2000.00,
      insuranceIncome: 2500.00,
    ...
    if (financialWithCalcs) {
      const expectedNetProduction = financialWithCalcs.production.toNumber() - 
                                   financialWithCalcs.adjustments.toNumber() - 
                                   financialWithCalcs.writeOffs.toNumber()
      
      const expectedTotalCollections = financialWithCalcs.patientIncome.toNumber() + 
                                      financialWithCalcs.insuranceIncome.toNumber()

      const netProductionMatch = Math.abs(financialWithCalcs.netProduction.toNumber() - expectedNetProduction) < 0.01
      const collectionsMatch = Math.abs(financialWithCalcs.totalCollections.toNumber() - expectedTotalCollections) < 0.01
    ...
    if (multiLocationProvider) {
      ...
      console.log(`   - Locations: ${multiLocationProvider.providerLocations.map(pl => 
        `${pl.location.name}${pl.isPrimary ? ' (Primary)' : ''}`
      ).join(', ')}`)
    }
...
async function main() {
  try {
    ...
    if (result.success) {
      ...
      console.log(`  - Provider-Location Relationships: ${result.summary?.providerLocationRelationships}`)
